...
namespace SimpleModernVideoPlayer
{
    class ParticleSystem
    {
        ...
        private void SpawnParticle()
        {
            ...
            for (int i = 0; i < particleCount; i++)
            {
                ...
                Particle p = new Particle
                {
                    ...
                    Velocity = new Vector2((float)(random.Next(-speed, speed) * 0.01), (float)(random.Next(-speed, speed) * 0.01)),
                };
        ...
        private void SetParticleOpacityAnimation(Ellipse p)
        {
            ...
            da.From = random.NextDouble();
            da.To = 0;
            da.Duration = new Duration(TimeSpan.FromMilliseconds(random.Next(2000, 5000)));
        ...
        public void ParticleRoamUpdate(Point mp)
        {
            foreach (Particle p in particles)
            {
                ...
                //速度为0判断
                if (p.Velocity.X <= float.MinValue) p.Velocity.X = (float)(random.Next(-speed, speed) * 0.01);
                if (p.Velocity.Y <= float.MinValue) p.Velocity.Y = (float)(random.Next(-speed, speed) * 0.01);
                    ...
                    if (s <= lineThreshold)
                    {
                        if (!p1.ParticleLines.ContainsKey(p2))
                        {
                            Line line = new Line()
                            {
                                ...
                                X2 = x2,
                                Y2 = y2,
                                Stroke = new SolidColorBrush(Windows.UI.Color.FromArgb(125, 255, 255, 255)),
                            };
